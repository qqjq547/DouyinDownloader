...
public class DownloadManager implements DownloadProgressListener {
    ...
    private DownloadManager() {
        ...
        outFile = new File(Environment.getExternalStorageDirectory()+"/11DyDownload/");
    ...
    public void start(String url) {
        ...
        Retrofit retrofit = new Retrofit.Builder()
                ...
                .baseUrl(CommonUtils.getBasUrl(ApiClient.API_SERVER_URL))
                .build();
        if (service == null) {
        ...
        subscribe = service.download("bytes=" + info.getReadLength() + "-", info.getUrl())
                ...
                .map(new Func1<ResponseBody, DownloadInfo>() {
                    ...
                    public DownloadInfo call(ResponseBody responseBody) {
                        try {
                            //写入文件
                            File file=new File(info.getSavePath()+"/"+CommonUtil.dateToString(new Date(),"yyyy-MM-dd hh-mm-ss")+".mp4");
                            if (file.exists()){
                                file.delete();
                            }
                            file.createNewFile();
                            FileUtil.writeCache(responseBody, file, info);
                        } catch (IOException e) {
                            Log.e("异常:", e.toString());
                        }
                        return info;
